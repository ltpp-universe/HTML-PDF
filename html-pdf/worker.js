...
process.on('message', async (message) => {
  ...
  await workerTask(
    ...
    message?.end,
    message?.paper_width,
    message?.paper_height,
    message?.paper_margin,
    thread_loc
  );
...
async function workerTask(
  ...
  paper_width = 1240,
  paper_height = 1754,
  paper_margin = { top: 0, right: 0, bottom: 0, left: 0 },
  thread_loc
) {
  ...
  for (let i = Math.max(0, start); i < end && i < all_dir_len; ++i) {
    ...
    try {
      ...
      await new Promise((resolve) => {
        pdf
          .create(fs.readFileSync(html_path, 'utf-8'), options)
          .toFile(new_path + '/' + pdf_name, function (err) {
            if (err) {
              Console.error(
                '【错误日志】【' +
                  (pdf_save_name ?? '未知文件名') +
