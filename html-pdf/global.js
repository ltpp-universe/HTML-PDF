 ...
 */
module.exports.copyFile = function (source_path, destination_dir, destination_file_name) {
    try {
        if (!fs.existsSync(destination_dir)) {
            fs.mkdirSync(destination_dir, { recursive: true });
        }
        if (fs.lstatSync(source_path)) {
            fs.copyFileSync(source_path, `${destination_dir}/${destination_file_name}`);
        }
    } catch (err) {
        Console.error(0, '【扫描目录出错】' + err);
    }
}
    ...
    try {
        if (fs.statSync(path)?.isDirectory()) {
            ...
            if (path.indexOf('.html') != -1) {
                Console.log(0, '【运行日志】扫描到HTML文件【' + (path ?? '未知文件名') + '】');
                return [path];
            }
            return [];
        }
...
module.exports.showEnvironment = function () {
    Console.log(0, `当前Node版本${process.version}`);
}